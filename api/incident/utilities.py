...
class validateIncident:
    ...
    def validate_createdBy(createdBy):
        return isinstance(createdBy, str) and \
         not re.search(r'[\s]', createdBy)

    @staticmethod
    def validate_type(type):
        return isinstance(type, str) and type == 'red-flag' \
         or type == 'intervention'
...
def change_status(incident_id):
    ...
    if not status:
        return jsonify({'status': 400,
                        'error': 'status field is either empty or missing'}), 400
